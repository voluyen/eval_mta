...
RESIDUAL_BASE="${BASE_PATH}/checkpoints/residual_mta"
DOWNLOAD_ROOT="${BASE_PATH}/checkpoints"
OUTPUT_ROOT="${BASE_PATH}/eval_outputs"
# discover = quét CHECKPOINT_ROOTS, chọn 1 ckpt/experiment; explicit = chỉ eval RESIDUAL_CKPTS
EVAL_MODE="${EVAL_MODE:-explicit}"
...
# )
# Danh sách checkpoint residual được bật.
# Muốn bỏ checkpoint nào thì thêm # ở ĐẦU dòng đó trong block EOF bên dưới.
# Ví dụ:
#   # results/gpt2/train/.../14290
# Dòng rỗng, dòng bắt đầu bằng #, và trailing comment dạng "  # note" sẽ bị bỏ qua.
mapfile -t RESIDUAL_CKPTS < <(
    sed -E \
        -e 's/[[:space:]]+#.*$//' \
        -e '/^[[:space:]]*($|#)/d' \
        -e 's/^[[:space:]]*//' \
...
collect_explicit_residual_ckpts() {
    local rel_path ckpt_dir
    local -a ckpts=()
    local missing=0

    for rel_path in "${RESIDUAL_CKPTS[@]}"; do
        ckpt_dir="${RESIDUAL_BASE}/${rel_path}"
        if [ -f "${ckpt_dir}/config.json" ] || [ -f "${ckpt_dir}/adapter_config.json" ]; then
            ckpts+=("${ckpt_dir}")
        else
            echo "[WARN] Không tìm thấy checkpoint local: ${rel_path}"
            echo "       Mong đợi: ${ckpt_dir}"
            missing=1
        fi
    done

    if [ "${#ckpts[@]}" -eq 0 ]; then
        echo "[ERROR] Không có checkpoint nào trong RESIDUAL_CKPTS tồn tại local."
        exit 1
    fi
...
download_checkpoints() {
    # To re-enable the old mta-dskd download, uncomment HF_REPO/DWA_CKPTS above
    # and this block:
    #
    # echo "======================================================"
    # echo " Download mta-dskd checkpoints"
    # echo " Repo       : ${HF_REPO}"
    ...
    #     --max-workers 4
    #
    # To re-enable AMID download, uncomment AMID_REPO above and this block:
    #
    # echo "======================================================"
...
get_output_rel() {
    ...
    case "${ckpt_dir}" in
        "${DOWNLOAD_ROOT}/residual_mta/results/"*)
            echo "residual_mta/${ckpt_dir#${DOWNLOAD_ROOT}/residual_mta/results/}"
            ;;
        "${DOWNLOAD_ROOT}/amid_mta/"*)
            echo "amid_mta/${ckpt_dir#${DOWNLOAD_ROOT}/amid_mta/}"
            ;;
        "${DOWNLOAD_ROOT}/rerun/"*)
            echo "rerun/${ckpt_dir#${DOWNLOAD_ROOT}/rerun/}"
            ;;
        "${DOWNLOAD_ROOT}/dwa_mta/"*)
            echo "dwa_mta/${ckpt_dir#${DOWNLOAD_ROOT}/dwa_mta/}"
            ;;
        *)
            echo "${ckpt_dir#${DOWNLOAD_ROOT}/}"
            ;;
    esac
}
...
eval_ckpt() {
    ...
    local device="$2"
    local ckpt_rel
    ckpt_rel="$(get_output_rel "${ckpt_dir}")"
...
echo ""
echo "======================================================"
echo " Resolve checkpoints to evaluate"
    ...
    for marker_name in ("config.json", "adapter_config.json"):
        for marker in ckpt_root.rglob(marker_name):
            ...
            if prev is None or priority > prev[0]:
                best[exp_dir] = (priority, ckpt_dir)

for _, ckpt_dir in sorted(best.values(), key=lambda item: str(item)):
    print(ckpt_dir)
PY
    )
    # Vẫn tôn trọng RESIDUAL_CKPTS trong discover mode:
    # checkpoint nào đã comment trong block RESIDUAL_CKPTS sẽ không được eval,
    # kể cả khi checkpoint đó còn tồn tại sẵn trong CHECKPOINT_ROOTS.
    declare -A ACTIVE_RESIDUAL_CKPT_SET=()
    for rel_path in "${RESIDUAL_CKPTS[@]}"; do
        ACTIVE_RESIDUAL_CKPT_SET["${RESIDUAL_BASE}/${rel_path}"]=1
    done

    FILTERED_CKPTS=()
    for ckpt_dir in "${CKPTS[@]}"; do
        if [ -n "${ACTIVE_RESIDUAL_CKPT_SET[${ckpt_dir}]:-}" ]; then
            FILTERED_CKPTS+=("${ckpt_dir}")
        fi
    done
    CKPTS=("${FILTERED_CKPTS[@]}")

    if [ "${#CKPTS[@]}" -eq 0 ]; then
        echo "Không tìm thấy checkpoint phù hợp trong các CHECKPOINT_ROOTS sau khi lọc theo RESIDUAL_CKPTS."
        exit 1
    fi
    printf "Tìm thấy %d checkpoint (discover mode, đã lọc theo RESIDUAL_CKPTS):\n" "${#CKPTS[@]}"
else
    mapfile -t CKPTS < <(collect_explicit_residual_ckpts)
    MISSING_CKPT=$?
    printf "Tìm thấy %d checkpoint (explicit RESIDUAL_CKPTS):\n" "${#CKPTS[@]}"
    if [ "${MISSING_CKPT}" -ne 0 ]; then
        echo "[WARN] Một số path trong RESIDUAL_CKPTS chưa có file local. Chạy lại download hoặc bỏ comment path đó."
    fi
fi
...
#         exit 1
#     fi
#     printf "Tìm thấy %d checkpoint (discover mode):\n" "${#CKPTS[@]}"
# else
#     mapfile -t CKPTS < <(collect_explicit_residual_ckpts)
#     MISSING_CKPT=$?
#     printf "Tìm thấy %d checkpoint (explicit RESIDUAL_CKPTS):\n" "${#CKPTS[@]}"
